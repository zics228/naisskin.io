...
<html lang="en">
	<head>
		...
		<title>Рефералы - X2DROP.COM</title>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$("#collect").on("click",function(){
					...
					success:function(data){
						try{
							...
							if(data.success){
								$("#avail").html(0);
								bootbox.alert(data.collected+" кредитов были зачислены на ваш счет!");
				...
				$("#changecode").on("click",function(e){
					e.preventDefault();
					bootbox.prompt("Введите новый промо-код:",function(result){                
						if(result){
							$.ajax({
								url:"/changecode",
								data:{"code":result},
								...
								success:function(data){
									try{
										data = JSON.parse(data);
										if(data.success){
											bootbox.alert("Код изменен на: "+data.code);
...
function saveSettings(){
	...
	if($("#settings_dongers").is(":checked")){
		$("#balance").html("пожалуйста, перезагрузите");
	}else{
		$("#balance").html("пожалуйста, перезагрузите");
	}
}
	...
	$.ajax({
		...
		success:function(data){		
			try{
				data = JSON.parse(data);
				if(data.success){
					bootbox.alert("Успех! Вам начислено "+data.credits+" кредитов.");					
				}else{
					bootbox.alert(data.error);
				}
			}catch(err){
		...
		error:function(err){
			bootbox.alert("AJAX error: "+err);
				...
				<div class='alert alert-success text-center'>Ваш реферальный код:  <b><span id='thecode'><?=$affiliates['code']?></span> - <a href='#' id='changecode'>обновить</a></b></div>     
    <table class="table">

    	<tr><td>Реферальный уровень</td><td><?=$affiliates['level']?></td></tr>
		<tr><td>Рефералов</td><td><?=$affiliates['visitors']?></td></tr>
		<tr><td>Активных рефералов</td><td><?=$affiliates['depositors']?></td></tr>
		<tr><td>Общая ставка</td><td><?=$affiliates['total_bet']?></td></tr>
		<tr><td>Общий доход</td><td><?=$affiliates['lifetime_earnings']?></td></tr>
		<tr><td>Доступно монет</td><td id='avail'><?=$affiliates['available']?></td></tr> 

    	</td></tr>
    </table>
<div class="text-right">
    <button class="btn btn-success btn-block" style="background: #5cb85c; color: #fff; width: 400px; margin: 0 auto;" id="collect">Получить монеты</button>
</div>
				<br><table id='referrals' class='table table-striped dataTable no-footer'><thead><th>SteamID</th><th>Общая ставка</th><th>Реферальный бонус</th></thead>
