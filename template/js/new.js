...
function finishRoll(m, tf) {
	$BANNER.html("Выпало число " + m.roll + "!");
...
function onMessage(msg) {
	...
	if (m.type == "preroll") {
		...
		MAX_BET = m.maxbet;
		chat("alert", "Минимальная ставка: " + m.minbet + " монет, " + "максимальная ставка: " + formatNum(MAX_BET) + " монет, " + " кол-во ставок за раунд: " + m.br + ", длительность раунда : " + m.accept + " сек, " + " чат: " + m.chat + " сек задержки. ");
        } else if (m.type == "bet") {
            if (showbets) {
                addBet(m.bet);
                $("#panel0-0-t .total").countTo(m.sums[0]);
...
function connect() {
	if(!SOCKET) {
		chat("italic", "Генерация токена...");
		var hash = getCookie('hash');
		if(!hash) {
			chat("italic", "Пожалуйста, авторизуйтесь на сайте через Steam.");
		} else {
			chat("italic", "Подключение к серверу...");
			SOCKET = io(HOST);
			SOCKET.on('connect', function(msg) {
				chat("italic", "Подключено!");
				SOCKET.emit('hash', hash);
			});
			SOCKET.on('connect_error', function(msg) {
				chat("italic", "Соединение разорвано...");
			});
		...
		console.log("Ошибка: Подключение уже существует.");
	}
}
...
$(document).ready(function() {
    ...
    $('.clearChat').on("click", function() {
        $('#chatArea').html("<div><b class='text-success'>Чат очищен!</b></div>")
    });
...
function changeLang(id) {
    ...
    chat("alert", "Перешли в комнату: " + $(".language > li").eq(id - 1).find("a").html())
}
