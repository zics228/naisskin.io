...
function loadLeft(opts) {
    ...
    inlineAlert("", "Загружаем предметы - пожалуйста подождите...");
        ...
        success: function(data) {
            try {
                ...
                if (data.success) {
                    ...
                    addPadding("#left", 6);
                    if (data.fromcache) {
                        inlineAlert("success", "Загружено " + count + " предметов из кэша - <a href=\"javascript:loadLeft('nocache')\">обновить инвентарь</a>");
                    } else {
                        inlineAlert("success", "Загружено " + count + " предметов..");
                    }
                } else {
                    inlineAlert("error", data.error);
                    if (data.count > 0) {
